...
def parse_action(response: str) -> dict[str, Any]:
    ...
    if response.startswith("do"):
        # First try: match do(action="...", message="...") with escaped quotes
        pattern = r'do\s*\(\s*action\s*=\s*"([^"]*)"\s*(?:,\s*message\s*=\s*"((?:[^"\\]|\\.|[\r\n])*)"\s*)?\)'
        match = re.search(pattern, response, re.DOTALL)
        
        # If pattern doesn't match, try manual parsing for complex messages with unescaped quotes
        if not match:
            # Find action="..." first
            action_match = re.search(r'action\s*=\s*"([^"]+)"', response)
            if action_match:
                action_name = action_match.group(1)
                # Find message="..." - look for message=" and find the matching closing quote before )
                message_match = re.search(r'message\s*=\s*"', response)
                if message_match:
                    start_pos = message_match.end()
                    # Find the last " before the closing )
                    end_pos = response.rfind('")', start_pos)
                    if end_pos > start_pos:
                        message = response[start_pos:end_pos]
                        # Unescape special characters
                        message = message.replace('\\"', '"').replace('\\n', '\n').replace('\\r', '\r').replace('\\\\', '\\')
                        action = {"_metadata": "do", "action": action_name, "message": message}
                        return action
        ...
        if match:
            ...
            if match.group(2):
                # Unescape special characters while preserving newlines
                message = match.group(2)
                message = message.replace('\\"', '"').replace('\\n', '\n').replace('\\r', '\r').replace('\\\\', '\\')
                action["message"] = message
            return action
        
        # Fallback to AST parsing (with newline escaping)
        try:
            escaped_response = response.replace('\n', '\\n').replace('\r', '\\r')
            tree = ast.parse(escaped_response, mode="eval")
